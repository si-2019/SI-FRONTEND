...
class UsmeniPoPredmetu extends Component {
    constructor(){
        super();
        this.state = {
            data: null,
    ...
    componentDidMount(){
        ...
        dataPredmetPoGodini.get(predmetId, godinaId, "Usmeni", datum).then( data => {
            this.setState(data);
        }).catch(err => {
            toast.error("Greska!");
        })
        ...
        return (this.state.data ? 
            <Fragment>
                ...
                <GrafikStavka
                    ...
                    nazivStavke={`Usmeni ${this.props.datum}`}
                    tipGrafika='Bar'
                />
            </Fragment> :
            <Spinner />)
...
export default UsmeniPoPredmetu;